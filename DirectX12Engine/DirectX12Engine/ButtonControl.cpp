...
void ButtonControl::Update(bool current)
{
    ...
    if (m_current)
    {
        ...
        {
            m_progress = Progress::FirstInterval;
            m_elapsedTime = 0.0f;
        }
        [[fallthrough]]; // フォールスルー属性

        case Progress::FirstInterval:
        {
            m_elapsedTime += DeltaTime;
            if (m_elapsedTime >= m_firstInterval)
            {
                m_progress = Progress::RepeatPressed;
                m_elapsedTime -= m_firstInterval;
            }
        }
        break;
        ...
        {
            m_progress = Progress::RepeatInterval;
            m_elapsedTime = 0.0f;
        }
        [[fallthrough]]; // フォールスルー属性
        ...
        {
            m_elapsedTime += DeltaTime;
            if (m_elapsedTime >= m_repeatInterval)
            {
                m_progress = Progress::RepeatPressed;
                m_elapsedTime -= m_repeatInterval;
            }
        }
        break;
        }
...
{
    return (m_progress == Progress::FirstPressed ||
        m_progress == Progress::RepeatPressed);
}
